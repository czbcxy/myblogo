...
<script>
    function select() {
        $.ajax({
            ...
            success: function (data) {
                ...
                for (j = 0; j < data.length; j++) {
                    ...
                    html = html + "<option value=" + data[j].name + ">" + data[j].name + "</option>"
                }
        ...
        $("#select").change(function () {
            ...
            var inputValue = $('#id_input_article_tags_in_modal').val();
            var t = inputValue.split(",");
            if (t.indexOf(tag) === -1) {
                var tags = inputValue + "," + tag;
                if (tags.startsWith(",")) {
                    tags = tags.substring(1);
                }
                $("#id_input_article_tags_in_modal").val(tags);
            }
        })
    };
    select();
